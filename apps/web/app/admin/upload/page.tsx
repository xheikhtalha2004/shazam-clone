...
export default function AdminUploadPage() {
  ...
  const canSubmit = title.trim() && artist.trim() && audioFile && !loading;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      // Validate file size under Vercel's 4.5 MB payload limit
      if (audioFile && audioFile.size > 4.5 * 1024 * 1024) {
        setError('File exceeds Vercel limits (max 4.5 MB). Please upload a compressed MP3 (e.g. 128kbps) or a shorter audio clip.');
        return;
      }
          ...
          {result ? (
            ...
            >
              ...
              <p style={{ textAlign: 'center', fontSize: '0.8125rem', color: 'var(--color-text-dim)' }}>
                Supported formats: MP3, WAV, FLAC, M4A, OGG, AAC · Max 4.5 MB (Vercel Limit)
              </p>
            </form>
          )}
      </main>
